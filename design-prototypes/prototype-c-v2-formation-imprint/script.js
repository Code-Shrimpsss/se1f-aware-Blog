(function () {
  ...
  const MEMORY_KEY = 'self-awake-c-v2-phase-one'
  ...
  function updateCoreDwell() {
    ...
    const target = window.innerWidth <= 760 ? { x: 0.51, y: 0.4, radius: 0.32 } : { x: 0.59, y: 0.485, radius: 0.29 }
  ...
  function updatePointer(clientX, clientY, active = true) {
    ...
    const imprintDistance = Math.hypot(pointer.normalizedX - 0.61, pointer.normalizedY - 0.61)
    if (phase === 'explore') body.classList.toggle('is-imprint-visible', imprintDistance < 0.29 || window.innerWidth <= 760)
  }
  ...
  function applyShotMode() {
    ...
    const aliases = {
      ...
      observation: 'explore',
      observe: 'explore',
      'desktop-observation': 'explore',
      'mobile-observation': 'explore',
    }
    const target = aliases[shot] || shot
    const allowed = ['core', 'core-observed', 'surface', 'storm', 'fragment-observed', 'freeze', 'impact', 'explore']
    const next = allowed.includes(target) ? target : 'core'
    const settings = {
      core: { progress: 1, time: 10600, x: 0.5, y: 0.5 },
      'core-observed': { progress: 1, time: 11200, x: 0.62, y: 0.46 },
      surface: { progress: 0.86, time: 13200, x: 0.55, y: 0.47 },
      storm: { progress: 0.76, time: 16800, x: 0.5, y: 0.48 },
      ...
      explore: { progress: 1, time: 20400, x: 0.61, y: 0.61 },
    }[next]
  ...
  function jumpTo(next) {
    const allowed = ['core', 'core-observed', 'surface', 'storm', 'fragment-observed', 'freeze', 'impact', 'explore']
    if (!allowed.includes(next)) throw new Error(`Unknown V2 state: ${next}`)
    shotMode = false
  ...
  function initialize() {
    try {
      ...
      console.error('Self-Awake V2 could not initialize its visual world.', error)
